...
case class DatabasePostgres(conn: Connection, tableName: String) extends Database {
  ...
  private val selectStatement: PreparedStatement = conn.prepareStatement(s"SELECT url FROM ${tableName} WHERE handle = ?")
  private val insertStatement: PreparedStatement = 
    conn.prepareStatement(s"INSERT INTO ${tableName} VALUES (?, ?, ?) ON CONFLICT (handle) DO NOTHING")
  private val selectAllstmnt: PreparedStatement = conn.prepareStatement(s"SELECT * FROM ${tableName};")
  ...
  //Checks if the URL is present in the database, and if yes, what is the handle associated with it.
